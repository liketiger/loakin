...
- 관심사를 분리하는 방법 중 Presentational-Container와 Custom Hook 중 Custom Hook을 적용해 분리할 예정
    - 커스텀 훅 방식이 더 함수형 방식에 적합하다고 판단

**💡 axios 적용 및 api 추상화**

- .json()으로 풀어줄 필요가 없이 코드수를 줄일 수 있는 장점이 있어 채택 예정.
- 현재 request-http와 useDB로 추출하여 중복 코드 제거와 추상화가 되어 있으나 axios를 적용하면서 한 단계 더 api 추상화 예정.

**💡 MongoDB 용량 관리**

- 무료 제공 용량이 512MB으로 언젠가는 초과할 수 도 있다는 위험부담이 있다는 점을 고려
- MongoDB에서 일정 시기마다 정해진 코드를 시행하는 기능이 확인 후 오래된 데이터 삭제하는 방법 사용 예정
...
- 종종 DB상의 문제가 아닌 클라이언트단에서 레이드 크루 삭제시 다른 크루까지 사라지거나 사라진 레이드가 나타나는 현상 발생
- 예상 원인
  - 전역 상태관리에서 스케쥴 상태의 일부 데이터를 가지는 레이드용 전역 상태를 따로 만들어 관리 
  - 그로인해 스케쥴 상태를 업데이트 하여도 레이드 상태를 사용하는 컴포넌트가 렌더링 X
- 해결 방안
  - 번거롭더라도 하나의 전역 상태를 이용하고 매번 정제하여 필요한 데이터만 사용하는 방식 채택 예정
- 유니크 ID를 DB에서 붙여주는 ID로 사용하다보니 클라이언트에서 이를 이용해 CRUD 작업을 할 경우 해당 ID가 부여되지 않은 채로 작업이 되는 현상 발생
- 작업 수행 후 다시 서버와 통신하여 적용된 ID를 받아오는 방식을 채택하고 그에 맞게 전역 상태 수정 예정

**💡 에러 핸들링 개선**

- 초기에 에러 핸들링 방법을 몰라 alert()로 확인
- 추후에 상황에 맞게 status code를 전송하고 이에 맞는 에러를 핸들링을 개선할 예정.

**💡 UX 개선을 위해 조직도 검색 기능 추가**

- 인원이 많아지면 조직도 페이지의 멤버 및 멤버별 캐릭터 수가 증가하여 스크롤 기능의 불편함을 예상하여 추후 검색기능 추가 예정
...
- [링크](https://velog.io/@liketiger/loakin-memoir)